...
**Rule learned the hard way:** never use `--accent` as small text color directly on `--paper`/`--bg` — the first mockup pass did exactly that and failed WCAG AA for most of the swatch options (a review agent caught it before publish). Always use accent as a **fill**, with fixed dark text `#241D14` on top, never `var(--ink)` (which flips light/dark and won't reliably contrast against a mid-tone accent in both themes).

**A second, related rule, learned building the real app:** that fixed-dark-text-on-fill trick only works for `--accent`/`--gold`, which stays similarly light in both themes by design. `--mint`/`--coral` (`--stamp-approve`/`--stamp-reject`) are the opposite - deliberately dark "ink" colors in light mode and brightened in dark mode, so they work well as *text on paper* (which is their main use: `text-mint`, `text-coral`, `border-mint`, `border-coral`) but neither a fixed light nor a fixed dark text color contrasts reliably against them *as a fill* in both themes at once. Don't fill a button or highlight with `bg-mint`/`bg-coral` solid - use a light tint (`bg-mint/15`, `bg-coral/15`) with `text-ink` (which does flip correctly) instead, or keep mint/coral to border+text only, as the stamp badges do. See `docs/solutions/`.
